...
public class OrderController {
    ...
    public ResponseEntity getAllOrders(
        ...
        String userId = jwtUtil.getEmailFromToken(bearerToken);
    ...
    public ResponseEntity getOrderById(
        ...
        String bearerToken = token.substring(7); // Remove 'Bearer ' from token
        String emailFromToken = jwtUtil.getEmailFromToken(bearerToken);
        Role role = jwtUtil.getRoleFromToken(bearerToken);


        OrderResponse order = orderService.findByIdForUser(id, emailFromToken, role);
